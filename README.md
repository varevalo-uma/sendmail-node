...
E-mail continues being a good solution for reciving info from many apps. Everybody has a mail client in his desktop, laptop, or smartphone, so you do not need additional tools for that purpose. So, why do we not use e-mail for sending updates about the status of a mobile robot that, for example, explores a building for hours :-)?.

This package contains a Python script that uses the Pythons as an SMTP client for sending the messages. We rely on well-known linux packages for low-level stuff such as [postfix](https://www.postfix.org/) and mailtools.
...
The node waits emails troughout the topic named "chatter". The 
